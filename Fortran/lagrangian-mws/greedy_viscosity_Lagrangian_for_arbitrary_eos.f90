...
CONTAINS
  ...
  SUBROUTINE greedy_lambda_arbitrary_eos(in_rhol,in_ul,in_el,in_pl,in_rhor,in_ur,in_er,in_pr,in_tol,no_iter,&
    ...
    !IF (no_iter) THEN
    pstar = p2
    CALL no_iter_update_lambda(ul,pl,al,gammal,ur,pr,ar,gammar,p2,lambda_max)
    RETURN
!!$    ELSE
!!$       !===Iterations
...
!!$    END IF
  ...
  SUBROUTINE no_iter_update_lambda(ul,pl,al,gammal,ur,pr,ar,gammar,p2,lambda_max)
    IMPLICIT NONE
    ...
    REAL(KIND=NUMBER) :: v11, v32, x, y, lambda_maxl, lambda_maxr, &
         tau_l, tau_r, tau_max, tau_min, tau_small
    ...
    lambda_max = max(x,y,epsilon*lambda_max)
