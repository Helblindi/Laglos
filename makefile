...
define LAGLOS_HELP_MSG

Laglos makefile targets:
...
make setup
   Build Laglos third party libraries: HYPRE, METIS and MFEM
   (By default MFEM will be compiled in parallel mode, but MFEM_BUILD=pcuda
    will allow a parallel CUDA build.)
make -j 4
   Build Laglos using the current configuration options from MFEM.
   (Laglos requires the MFEM finite element library, and uses its compiler and
    linker options in its build process.)
make status
   Display information about the current configuration.
make install PREFIX=<dir>
   Install the Laglos executable in <dir>.
make clean
   Clean the Laglos executable, library and object files.
make distclean
   In addition to "make clean", remove the local installation directory and some
   run-time generated files.
make style
   Format the Laglos C++ source files using the Artistic Style (astyle) settings
   from MFEM.
...
Laglos_FLAGS = $(CPPFLAGS) $(CXXFLAGS) $(MFEM_INCFLAGS)
...
CCC = $(strip $(CXX) $(Laglos_FLAGS) $(if $(EXTRA_INC_DIR),-I$(EXTRA_INC_DIR)))

Laglos_LIBS = $(MFEM_LIBS) $(MFEM_EXT_LIBS)
LIBS = $(strip $(Laglos_LIBS) $(LDFLAGS))
...
Laglos: $(OBJECT_FILES) $(CONFIG_MK) $(MFEM_LIB_FILE)
	$(MFEM_CXX) $(MFEM_LINK_FLAGS) -o Laglos $(OBJECT_FILES) $(LIBS)

all:;@$(MAKE) -j $(NPROC) Laglos
...
# test: Laglos
# 	@$(call mfem-test,$<, $(RUN_MPI_4), Laglos miniapp,\
...
install: Laglos
	mkdir -p $(PREFIX)
	$(INSTALL) -m 750 Laglos $(PREFIX)

help:
	$(info $(value Laglos_HELP_MSG))
	@true

status info:
	$(info MFEM_DIR     = $(MFEM_DIR))
	$(info Laglos_FLAGS = $(Laglos_FLAGS))
	$(info Laglos_LIBS  = $(value Laglos_LIBS))
	$(info PREFIX       = $(PREFIX))
	@true
...
# Laglos checks template - Default arguments
ECHO=echo
SED=sed -e
ranks=1
dims=2 3
...
# Laglos checks template - Targets
define Laglos_checks_template
.PHONY: Laglos_$(1)_$(2)_$(3)_$(4)
Laglos_$(1)_$(2)_$(3)_$(4): Laglos
	$(eval name=Laglos-x$(4)-p$(1)-$(2)D$(word $(3),$(options)))
	$(eval command=$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(4) ./Laglos $(OPTS) -p $(1) -dim $(2) $(shell echo $(word $(3),$(options))|$(SED) "s/-/ -/g"|$(SED) "s/_/ /g"))
...
endef
# Generate all Laglos checks template targets
$(foreach p, $(problems), $(foreach d, $(dims), $(foreach o, $(optioni), $(foreach r, $(ranks),\
	$(eval $(call Laglos_checks_template,$(p),$(d),$(o),$(r)))))))
# Output info on all Laglos checks template targets
#$(foreach p, $(problems), $(foreach d, $(dims), $(foreach o, $(optioni), $(foreach r, $(ranks),\
#   $(info $(call Laglos_checks_template,$(p),$(d),$(o),$(r)))))))
checks: Laglos
checks: |$(foreach p,$(problems), $(foreach d,$(dims), $(foreach o,$(optioni), $(foreach r,$(ranks), Laglos_$(p)_$(d)_$(o)_$(r)))))
...
tests:
	...
	./Laglos -p 0 -dim 2 -rs 3 -tf 0.75 -pa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 21 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 0 -dim 3 -rs 1 -tf 0.75 -pa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 21 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 1 -dim 2 -rs 3 -tf 0.8 -pa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 18 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 1 -dim 3 -rs 2 -tf 0.6 -pa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 18 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 2 -dim 1 -rs 5 -tf 0.2 -fa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 18 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 3 -m data/rectangle01_quad.mesh -rs 2 -tf 3.0 -pa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 18 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 3 -m data/box01_hex.mesh -rs 1 -tf 5.0 -pa -vs 100 | tee RUN.dat
	cat RUN.dat | tail -n 18 | head -n 1 | \
	awk '{ printf("step = %04d, dt = %s |e| = %.10e\n", $$2, $$8, $$11); }' >> RESULTS.dat
	$(MFEM_MPIEXEC) $(MFEM_MPIEXEC_NP) $(MFEM_MPI_NP) \
	./Laglos -p 4 -m data/square_gresho.mesh -rs 3 -ok 3 \
