...
class ElasticTest: public ProblemBase
{
private:
   /*********************************************************
   ...
   string _indicator = "ElasticTest";

   double rho= 2.7E3, p = 1.E5, vD = 0., vR = 1.;
   const double p_inf = 2.15E10;
   const double _mu = 2.6E10;

public:
   ElasticTest(const int &_dim) : ProblemBase(_dim)
   {
   ...
   void v0(const Vector &x, const double & t, Vector &v) const  override
   {
      ...
      if (t < 1e-12)
      {
         // Prescribe a little velocity in a small region
         if (x[0] < -1. && x[0] > -1.5 && x[1] < 0.5 && x[1] > -0.5)
         {
            v[0] = vR;
         }
      }
