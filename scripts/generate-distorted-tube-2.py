...
def main():
   ...
   filename = home_dir + "data/distorted-tube-2.mesh"
   f = open(filename, "w")
   ...
   for i in range(0, nx_gridpoints):
      ...
      for j in range(0, ny_gridpoints):
         y = y_arr[j]
         # Maintain straight boundaries
         if (i == 0 or i == nx_gridpoints - 1):
            xt = x
         else:
            if j > (ny_gridpoints - 1) / 2:
               jti = ny_gridpoints - 1 - j
            elif j < (ny_gridpoints - 1) / 2:
               jti = j
            else:
               jti = (ny_gridpoints - 1) / 2
               
            xt = x + jti * .005

         
         if (y != 0 and y != .1):
            if (i > (nx_gridpoints - 1) / 2):
               itj = nx_gridpoints - 1 - i
            elif i < (nx_gridpoints - 1) / 2:
               itj = i
            else:
               itj = (nx_gridpoints - 1) / 2
