   ...
   _l2x_gf(&L2V)
{
   ...
   if (use_elasticity)
   {
      elastic = new Elastic(dim, qdata, H1, ir);
      pb->SetElasticEOSType(elastic_eos);
      pb->SetElastic(*elastic);
      pb->set_rho0_vec(rho0_gf);
   }
...
void LagrangianLOOperator::ComputeESheerGF(ParGridFunction &e_sheer_gf) const
{
   ...
   for (int e = 0; e < NDofs_L2; e++)
   {
      int el_i = L2.GetElementForDof(e);
      int attr = pmesh->GetAttribute(el_i);
      e_sheer_gf[e] = pb->e_shear(e, attr);
   }
}
...
void LagrangianLOOperator::MultUnlimited(const Vector &S, Vector &dS_dt) const
{
   ...
   //    std::bind(&ProblemBase::v0, pb, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
   // VectorFunctionCoefficient v_coeff(dim, v0_static);
   // v_coeff.SetTime(this->GetTime());
   // dxdt_gf.ProjectCoefficient(v_coeff);
...
*  of size NE x NE where the nonzero elements are for each element row
   ...
   for (int dof_it = 0; dof_it < NDofs_L2; dof_it++)
   {
      ...
      for (int j = 0; j < row.Size(); j++)
      {
         ...
         if (c_norm > 1.e-12)
         {
            ...
            if (use_elasticity)
            {
               int el_l = L2.GetElementForDof(dof_it);
               int left_attr = pmesh->GetAttribute(el_l);
               esl = pb->e_shear(dof_it, left_attr);
               int el_j = L2.GetElementForDof(cj);
               int right_attr = pmesh->GetAttribute(el_j);
               esr = pb->e_shear(cj, right_attr);
            }
...
void LagrangianLOOperator::ComputeKidderAvgDensityAndEntropy(const Vector &S, double &avg_density, double &avg_entropy)
{
   ...
   for (int cell_it = 0; cell_it < NE; cell_it++)
   {
      ...
      int el = L2.GetElementForDof(cell_it);
      int attr = pmesh->GetAttribute(el);
      double density = 1. / U[0]; 
      double e_shear = 0.;

      if (use_elasticity) { e_shear = pb->e_shear(el, attr); }
      double sie = pb->specific_internal_energy(U, e_shear);
      double pressure = pb->pressure(density, sie, attr);
...
****************************************************************************************************/
void LagrangianLOOperator::GetStateVector(const Vector &S, const int &index, Vector &U) const
{
   // Ensure index is validAdd commentMore actions
   if (index >= NDofs_L2) { MFEM_ABORT("Index out of range.\n"); }
   ...
   U[0] = sv_gf.Elem(index);

   // Retrieve index velocity 
   // Here the corresponding gridfunction is a stacked vector:
   // Ex: x1, x2, x3, x4, y1, y2, y3, y4, z1, z2, z3, z4
   for (int i = 0; i < dim; i++)
   {
      int _ind = index + i*NDofs_L2;
      assert(_ind < v_gf.Size());
      U[i+1] = v_gf.Elem(_ind);
   }

   // Retrieve index specific total energy
   U[dim+1] = ste_gf.Elem(index);
}
...
****************************************************************************************************/
void LagrangianLOOperator::SetStateVector(Vector &S, const int &index, const Vector &U) const
{
   // Ensure index is validAdd commentMore actions
   if (index >= NDofs_L2) { MFEM_ABORT("Index out of range.\n"); }
...
void LagrangianLOOperator::GetLocalCij(const int &i, const int &j, Vector &cij) const
{
   ...
   /* Get y and z vals, if applicable */
   if (dim > 1)
   {
                                                    ...
                                                    const string &output_file_suffix)
{
   ...
   for (int i = 0; i < NDofs_L2; i++)
   {
      ...
      double rho = 1. / U[0];
      double e_shear = 0.;
      if (use_elasticity) { e_shear = pb->e_shear(i, attr); }
      double sie = pb->specific_internal_energy(U, e_shear);
...
void LagrangianLOOperator::ComputeCellAverageVelocityAtNode(const Vector &S, const Vector &S_old, const int node, const bool &is_weighted, int td_flag, Vector &node_v)
{
   ...
   for (int cell_it = 0; cell_it < cells_length; cell_it++)
   {
      ...
      case 0: // S_OLD
      {
         GetStateVector(S_old, el_index, Uc);
         pb->velocity(Uc, Vcell);
         break;
      }
      case 1: // S_NEW 
      ...
      {
         GetStateVector(S_old, el_index, Uc);
         GetStateVector(S, el_index, Ucnp1);
...
void LagrangianLOOperator::SolveHiOpDense(const Vector &S, const Vector &S_old, const int & target_option, const double &t, const double &dt, ParGridFunction &mv_gf_l)
{
   ...
   OptimizedMeshVelocityProblemDense omv_problem(dim, geom, V_target, massvec, x_gf, NE, dt, xmin, xmax);
